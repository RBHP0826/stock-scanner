...
print(f"총 {len(yf_tickers)}개 종목 중 100개 yfinance 다운로드 시작...")
start_time = time.time()
chunk = yf_tickers[:100]
df_chunk = yf.download(chunk, start="2026-06-17", end="2026-06-18", group_by='ticker', threads=True, progress=False)
end_time = time.time()
print(f"100개 다운로드 완료! 소요시간: {end_time - start_time:.2f}초")
if not df_chunk.empty:
    print("df_chunk shape:", df_chunk.shape)
    # 첫번째 종목의 데이터 확인
    first_ticker = chunk[0]
    if first_ticker in df_chunk.columns.levels[0]:
        print(f"--- {first_ticker} ---")
        print(df_chunk[first_ticker].head())
else:
