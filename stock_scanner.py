...
class StockScanner:
    ...
    def get_historical_data(self, symbol, market='KR', days=365):
        ...
        df = None
        if market in ['KR', '한국 (KRX)', 'KRX']:
            try:
                df = fdr.DataReader(symbol, start_date)
            except Exception as e:
                logger.warning(f"FinanceDataReader failed for {symbol}, trying pykrx fallback: {e}")
                try:
                    from pykrx import stock
                    start_pykrx = (datetime.datetime.now() - datetime.timedelta(days=days)).strftime('%Y%m%d')
                    today_pykrx = datetime.datetime.now().strftime('%Y%m%d')
                    df_pykrx = stock.get_market_ohlcv_by_date(start_pykrx, today_pykrx, symbol)
                    if df_pykrx is not None and not df_pykrx.empty:
                        df_pykrx = df_pykrx.rename(columns={
                            '시가': 'Open', '고가': 'High', '저가': 'Low', '종가': 'Close', '거래량': 'Volume'
                        })
                        df = df_pykrx
                except Exception as ex:
                    logger.error(f"pykrx fallback failed for {symbol}: {ex}")
        elif market in ['US', '미국 (US)']:
            try:
                df = fdr.DataReader(symbol, start_date)
            except Exception as e:
                logger.warning(f"FinanceDataReader failed for US symbol {symbol}, trying yfinance: {e}")
                try:
                    ticker = yf.Ticker(symbol)
                    df_yf = ticker.history(start=start_date)
                    if df_yf is not None and not df_yf.empty:
                        df = df_yf
                except Exception as ex:
                    logger.error(f"yfinance fallback failed for {symbol}: {ex}")
        ...
        try:
            ...
            if market in ['KR', '한국 (KRX)', 'KRX']:
                if not (len(symbol) == 6 and symbol.isdigit()):
                    found_code, found_market = self.find_symbol_by_name(symbol)
                    if found_code:
                        resolved_symbol = found_code
                    else:
                        logger.error(f"종목 코드 자동 변환 실패 (KR): {symbol}")
                        return None
            elif market in ['US', '미국 (US)']:
                if len(symbol) > 5 or any(c.islower() for c in symbol) or ' ' in symbol:
                    found_code, found_market = self.find_symbol_by_name(symbol)
                    if found_code:
                        resolved_symbol = found_code
                    else:
                        logger.error(f"종목 코드 자동 변환 실패 (US): {symbol}")
                        return None
            elif market in ['COIN', '암호화폐 (Upbit)']:
                if not symbol.startswith('KRW-'):
                    ...
                    if found_code:
                        ...
                        logger.error(f"종목 코드 자동 변환 실패 (COIN): {symbol}")
                        return None
            
            df = self.get_historical_data(resolved_symbol, market, 365)
